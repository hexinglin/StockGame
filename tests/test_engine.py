...
class TestDelete:
    def test_delete_cleans_all_round_data(self, running_round, engine, app):
        """物理删除一局：轮次/委托/成交行 + Redis 快照全清空，game_days 不受影响"""
        r, ctx = running_round
        ...
        with app.app_context():
            ...
            # game_days 行情行不受轮次删除影响（日期选择/管理仍可用）
            assert GameDay.query.filter_by(
                code=r.code, trade_date=r.trade_date,
                data_source=r.data_source).first() is not None
...
class TestGameDayGuard:
    """game_days 昨收防线：last_close 无效（空/0）的异常日行情拒绝入库"""

    def test_refresh_day_rejects_invalid_last_close(self, engine, app):
        """hint 无效 + 库内无旧值 + stock_kline 查不到 → 拒绝写入、零写入"""
    ...
    def test_refresh_day_valid_last_close_writes(self, engine, app):
        ...
        with app.app_context():
            ...
            try:
                ...
                row = GameDay.query.filter_by(code=code, trade_date=date,
