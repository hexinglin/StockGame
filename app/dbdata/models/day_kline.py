"""
日行情 ORM 模型 — 与 tick 表对齐的天维度真实行情（开局原始数据源）
...
- 本表在快照入库（上传/批量写入/迁移）时同步聚合写入当日终值，数据与 tick 表
  对齐（同 (code, trade_date) 对账）：high=max(各快照滚动 high)/low=min(low)/
...
- game_days 为游戏选择/管理层（可开局日、轮次计数），is_complete 由本表派生
  同步，开局所需原始数据（昨收/今开/OHLC/对齐信息）一律从本表读取。
"""
